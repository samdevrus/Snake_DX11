						   ...
						   D3DXMATRIX baseViewMatrix)
{
	...
	result = InitializeSentence(&m_sentence3, 255, device);
	if(!result)
	{
		return false;
	}
...
bool TextClass::SetHp(int hp, ID3D11DeviceContext* deviceContext)
{
	...
	strcpy_s(dataString, "Life: ");
	strcat_s(dataString, tempString);

	result = UpdateSentence(m_sentence1, dataString, 10, 10, 0.0f, 1.0f, 0.0f, deviceContext);
	if(!result)
	{
		return false;
	}
...
bool TextClass::SetPoints(int points, ID3D11DeviceContext* deviceContext)
{
	char tempString[16];
	char dataString[30];
	bool result;
	...
	// Setup the X rotation string.
	_itoa_s(points, tempString, 10);
	strcpy_s(dataString, "Points: ");
	strcat_s(dataString, tempString);

	result = UpdateSentence(m_sentence2, dataString, 10, 30, 0.0f, 1.0f, 0.0f, deviceContext);
	if(!result)
	{
		return false;
	}
...
bool TextClass::SetInformation(char dataString[255], ID3D11DeviceContext* deviceContext)
{	HRESULT result;
	result = UpdateSentence(m_sentence3, dataString, 10, 50, 0.0f, 1.0f, 0.0f, deviceContext);
	if(!result)
	{
		return false;
	}
